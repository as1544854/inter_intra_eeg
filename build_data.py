...
def load_edf_extract_seizures_v152(base_dir, save_data_dir, data_dict):
    """
    从EDF文件中加载数据并提取特定患者的癫痫发作数据。

    参数:
    - base_dir: 基础目录路径，其中包含所有患者的EDF文件。
    - save_data_dir: 保存提取的癫痫发作数据的目录路径。
    - data_dict: 包含患者癫痫发作信息的字典，其中每个键是癫痫发作类型，对应的值是一个癫痫发作列表，
      每个癫痫发作包括患者ID、文件名（.tse格式）和发作的开始及结束时间。

    返回值:
    - seizure_data_dict: 一个收集所有患者癫痫发作数据的字典，默认每个键是一个空列表。
    """
...
def gen_raw_seizure_pkl(args,tuh_eeg_szr_ver, anno_file):
    base_dir = args.base_dir#'E:\\dataSet\\TUSZ'

    save_data_dir = os.path.join(args.save_data_dir, tuh_eeg_szr_ver, 'raw_seizures')#args.save_data_dir  E:\dataSet\tusz_save
    if not os.path.exists(save_data_dir):
        os.makedirs(save_data_dir)

    raw_data_base_dir = os.path.join(base_dir, tuh_eeg_szr_ver)
    szr_annotation_file = os.path.join(raw_data_base_dir, '_DOCS', anno_file)#anno_file:seizures_v36r.xlsx
...
def main():
    ...
    parser.add_argument('--base_dir', default='E:\dataSet\TUSZ',
                        help='path to raw seizure dataset')
    parser.add_argument('--save_data_dir',
                        default='E:\dataSet\TUSZ',
                        help='path to save processed data')
    # else:
